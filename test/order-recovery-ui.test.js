...
test("운명 완전개봉 상품은 외부 리포트 재접수·상태 조회·보기 흐름을 가진다", () => {
  ...
  // 결제 완료 대기 화면: 3초 폴링 + 진행 프로그레스 + 완성 시 같은 탭 자동 오픈.
  assert.match(app, /beginExternalWait\(orderId\)/);
  assert.match(app, /intervalMs:\s*3000/);
  assert.match(app, /window\.location\.assign\(url\)/);
  assert.match(html, /data-report-progress/);
  assert.match(html, /data-progress-fill/);
  // 완성된 리포트는 클릭 즉시(비동기 대기 없이) 새 탭으로 연다.
  assert.match(app, /window\.open\(readyUrl, "_blank", "noopener"\)/);
  // 보관함의 외부 리포트 항목은 내부 분석 화면 대신 온라인뷰 리포트를 연다.
  assert.match(app, /item\.externalReport \|\| item\.reportUrl/);
