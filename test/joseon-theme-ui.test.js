...
test("홈은 흑야 조선 미스터리 세계관과 강무영 캐릭터를 전면에 건다", () => {
  ...
  assert.match(html, /assets\/generated\/banners\/heukya-hero/);
...
test("모든 주요 상품은 조선 테마 썸네일과 사극풍 상품 카피를 사용한다", () => {
  for (const asset of [
    "heukya-saju-reading.jpg",
    "heukya-compatibility.jpg",
    "heukya-fortune-cycle.jpg",
    "heukya-dark-mudang.jpg",
    "heukya-auspicious-date.jpg",
    "heukya-year-wheel.jpg",
    "heukya-consult-chat.jpg",
    "heukya-daily-note.jpg",
  ]) {
...
test("운영 런타임 설정도 흑야 테마 상품명과 이미지를 기본값으로 갱신한다", () => {
  ...
  assert.match(migration, /heukya-hero\.jpg/);
  assert.match(migration, /heukya-dark-mudang\.jpg/);
