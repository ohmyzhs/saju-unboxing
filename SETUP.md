...
3. 내 코드의 **`supabase/schema.sql`** 파일 **전체를 복사 → 붙여넣기 → `RUN`**
   ...
   - 기존 설치도 다시 실행해야 회원 포인트 테이블·원자적 차감 함수·주문 컬럼이 추가됩니다.
...
만세력은 프로젝트에 포함된 자체 엔진이 계산합니다. 별도 SAJULAB 키 없이
한국표준시 기준 원국·십성·용신·신살·대운·세운·월운 데이터를 생성합니다.

회원 포인트는 고객 결제수단입니다. 로그인 회원은 마이페이지에서 3개 고정
티어로 충전하고 분석 결제 때 전액 또는 일부를 사용할 수 있습니다.
...
| 분석 결제 시 **"포인트가 부족합니다"** | 사용 포인트를 줄이거나 마이페이지에서 충전 |
| 포인트 메뉴가 **사용 준비 중** | 최신 `supabase/schema.sql`을 SQL Editor에서 다시 실행 |
